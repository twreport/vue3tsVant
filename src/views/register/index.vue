    ...
    <div class="register-box">
        ...
        <van-form @submit="onSubmit">
            <van-cell-group inset>
                ...
                <van-field v-model="username" label="用户" name="用户" placeholder="用户名" :rules="[
                ...
                <van-field v-model="password2" type="password" name="重复密码" label="重复密码" placeholder="重复密码"
                    :rules="[{ required: true, message: '请填写密码' }, { validator, message: '两次密码输入不一致' }]" />
            </van-cell-group>
...
const onSubmit = () => {
    loginApi
        .register(username.value, password1.value)
        .then(res => {
            console.log(res)
            if ('err' in res) {
